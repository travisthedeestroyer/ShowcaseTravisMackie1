...
function PipelineNode({ icon, name, model, title }: NodeProps) {
  return (
    <div
      className="flex-shrink-0 w-[140px] glass-panel p-5 text-center relative group hover:bg-black/5 transition-colors cursor-default"
      title={title}
    >
      <span className="text-3xl mb-3 block opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all">{icon}</span>
...
const Arrow = () => (
  <div className="flex-shrink-0 w-10 flex items-center justify-center text-black/20 text-xl">
    →
  </div>
);
...
export function Pipeline() {
  return (
    <section id="pipeline" className="px-6 max-w-[1200px] mx-auto w-full">
      ...
      <div className="hidden sm:flex items-center gap-0 overflow-x-auto pb-8 scrollbar-hide">
        ...
        <div className="flex-shrink-0 flex flex-col gap-3 relative glass-panel p-4 bg-black/[0.02]">
          ...
          <div className="flex flex-col gap-3">
            <PipelineNode icon="✍️" name="Lyric Writer" model="Gemini 2.5 Pro" title="Writes lyrics matching vocal style and theme" />
            <PipelineNode icon="🥁" name="Beat Generator" model="Lyria Pro / Clip" title="Generates beat via Lyria Pro or algorithmic fallback" />
            <PipelineNode icon="🎛️" name="Mix Engineer" model="Gemini 2.5 Flash" title="Plans EQ, compression, reverb, stereo width" />
          </div>
        </div>
...
function MobilePipelineRow({ icon, name, model }: { icon: string; name: string; model: string; title: string }) {
  return (
    <div className="flex items-center gap-4 glass-panel p-4 hover:bg-black/5 transition-colors">
