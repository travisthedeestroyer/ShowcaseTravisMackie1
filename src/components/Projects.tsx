...
function FreshKidsHighlights() {
  ...
  return (
    <div className="relative w-full aspect-video bg-black rounded-xl overflow-hidden ring-1 ring-black/5 shadow-2xl select-none group/vp">
      <video
        ref={videoRef}
      ...
      <div className={`absolute bottom-0 left-0 right-0 z-30 transition-all duration-300 ${!isPlaying ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2 group-hover/vp:opacity-100 group-hover/vp:translate-y-0"}`}>
        <div
          ref={progressRef}
        ...
        >
          ...
          {CHAPTERS.map((ch, i) => (
            <div
              key={i}
              className="absolute top-0 w-px h-full opacity-60"
...
function VideoDemo() {
  ...
  return (
    <div className="relative w-full aspect-video bg-black rounded-xl overflow-hidden ring-1 ring-black/5 shadow-2xl group/video cursor-pointer" onClick={togglePlay}>
      <video
        ref={videoRef}
        src="/remix-demo.mp4"
        className="w-full h-full object-cover"
      ...
      <div className={`absolute inset-0 flex items-center justify-center transition-opacity bg-black/30 backdrop-blur-[2px] ${isPlaying ? "opacity-0 group-hover/video:opacity-100" : "opacity-100"}`}>
        <button className="w-16 h-16 rounded-full bg-white text-black flex items-center justify-center hover:scale-105 transition-transform shadow-xl">
...
function ProjectCard({ title, subtitle, description, features, tech, link, media }: ProjectCardProps) {
  return (
    <motion.div
    ...
    >
      <div className="flex-1 p-8 sm:p-12 flex flex-col justify-center relative z-10">
        <div className="font-mono text-xs text-blue-600 uppercase tracking-widest mb-4">{subtitle}</div>
        ...
        {link && (
          <a href={link} target="_blank" rel="noopener noreferrer" className="absolute top-8 right-8 w-12 h-12 rounded-full bg-black/5 border border-black/10 flex items-center justify-center text-ink hover:bg-ink hover:text-white transition-all">
  ...
  return (
    <section id="work" className="px-6 max-w-[1200px] mx-auto w-full">
      <div className="mb-16 sm:mb-24 text-center">
        <h2 className="font-display text-[clamp(3rem,8vw,5rem)] font-bold leading-none text-ink mb-6">
          SELECTED <span className="text-gradient">WORKS</span>
        </h2>
        <p className="text-lg text-ink-dim font-light max-w-[600px] mx-auto">
      ...
      <div className="flex flex-col gap-8 sm:gap-12">
        ...
        <ProjectCard
          ...
          description="Full-stack AI music production platform with 6-agent parallel processing pipeline. Record vocals and receive a complete mastered song — AI-generated lyrics via Gemini 2.5, professional beat production through Lyria Pro API, automated mixing plan generation, and FFmpeg-powered mastering chain."
          features={[
